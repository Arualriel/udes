...
sol=solve(prob,Vern7(),abstol=1e-11,reltol=1e-10)
...
prob = ODEProblem(dudt2_,u0,(0f0,ttrain),nothing)
...
function loss(params)
    ...
    predict=Array(solve(lprob,Vern7(),saveat=t[t.<=ttrain],abstol=1e-6,reltol=1e-6))
...
function losslv(p)
    ...
    pred = Array(solve(_prob,Vern7(),saveat=t[t .<= ttrain],abstol=1e-6,reltol=1e-6))
